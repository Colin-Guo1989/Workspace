 ...
 * way
 *这些被设计为不经常更新。对于高频数据，考虑使用驱动输入或添加到机器人调试数据。
 *控制参数:单个值，双精度，浮点数，vec3，或s64。每个控制参数必须有一个惟一的名称，
 *控制参数知道它们的类型，以及它们是否已初始化。控制参数必须通过从文件读取、从LCM读取或其他方式进行初始化
...
/*!
 * Data types supported for control parameters 控制参数支持的数据类型
 */
...
/*!
 * Pointer to control parameter data 控制参数数据的指针
 */
