...
/*Eigen::Matrix<T, 18, 1> _xhat;//状态估计值 [p v p1 p2 p3 p4] 世界坐标下
  ...
  _ps.setZero();
  _vs.setZero();
  //状态转移矩阵，计算K+1时刻状态值X[k+1] 自己写出来算下就知道 式21 22 23
  //p[k+1]=p[k]+v*dt
  //v[k+1]=v[k]+a*dt
  _A.setZero();
...
//trust 在支撑时一般为1 摆动时为0
    if (phase < trust_window) {//在支撑相最开始这一段时间和整个摆动过程
      trust = phase / trust_window;
    } 
	else if (phase > (T(1) - trust_window)) {//在支撑相最后一段时间
	...
	//式24（自己标的）
//处理后的
    _ps.segment(i1, 3) = -p_f;//足端位置在世界坐标系描述 固结在机身坐标系在和世界坐标系方向相同的坐标系下坐标
    _vs.segment(i1, 3) = (1.0f - trust) * v0 + trust * (-dp_f);//足端速度在世界坐标系描述 摆动足数据为估计的机身速度，支撑足数据为测量数据
  ...
  //卡尔曼滤波
  _xhat = _A * _xhat + _B * a;//状态预测方程 式21 22 23
  ...
  Eigen::Matrix<T, 28, 1> ey = y - yModel;//误差 (Z-H*X) 
  
  Eigen::Matrix<T, 28, 28> S = _C * Pm * Ct + R;//(H*P*H^t+R)^-1

  // todo compute LU only onc
  Eigen::Matrix<T, 28, 1> S_ey = S.lu().solve(ey);//求得 (H*P*H^t+R)^-1*(Z-H*X)
  
  _xhat += Pm * Ct * S_ey;//X+P*H(H*P*H^t+R)^-1*(Z-H*X)=X+K*(Z-H*X)

  Eigen::Matrix<T, 28, 18> S_C = S.lu().solve(_C);//求得 (H*P*H^t+R)^-1*H
  
  _P = (Eigen::Matrix<T, 18, 18>::Identity() - Pm * Ct * S_C) * Pm;//最佳估计不确定性协方差P-P*H*(H*P*H^t+R)^-1*P=P-K*H*P

  Eigen::Matrix<T, 18, 18> Pt = _P.transpose();//??不知道干啥
  _P = (_P + Pt) / T(2);//??

  if (_P.block(0, 0, 2, 2).determinant() > T(0.000001)) {//合理化
