...
public:
  WBC(size_t num_qdot) : num_act_joint_(num_qdot - 6), num_qdot_(num_qdot)
  {
    ...
    Sa_.block(0, 6, num_act_joint_, num_act_joint_).setIdentity(); //即乘状态向量即将浮动基座变量部分置为0
    Sv_.block(0, 0, 6, 6).setIdentity();                           //即乘状态向量即将关节变量部分置为0
  }
  virtual ~WBC() {}
...
protected:
  ...
  DMat<T> Sa_; // Actuated joint 12*18   即乘状态向量即将浮动基座变量部分置为0 
