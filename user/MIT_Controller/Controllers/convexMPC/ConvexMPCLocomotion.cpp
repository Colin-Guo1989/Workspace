...
//足端位置在世界坐标下 机身坐标+机身旋转矩阵^T*（髋关节在机身下坐标+腿在髋关节下坐标）（机身坐标下足端标转换倒和机体固接的平行于世界的坐标系）+机身位置
  for(int i = 0; i < 4; i++) {
...
//设置摆动足轨迹相关 设置抬腿高度
    ...
    Vec3<float> pYawCorrected = 
      coordinateRotation(CoordinateAxis::Z, -_yaw_turn_rate* stance_time / 2) * pRobotFrame;//由于不是标准旋转矩阵所以这么写
...
//世界坐标系下hip坐标 以剩余摆动时间内匀速运动来估计 即落脚时世界坐标下髋关节坐标
...
///*************************************摆动腿控制*************************************///
    ...
    //获得轨迹点
      ...
      // Update for WBC 为wbc控制更新参数  //将相关数据统统暴露出 可供外部读取
...
///*************************************为WBC更新*************************************///
  ...
  // Update For WBC 将相关数据统统暴露出 可供外部读取
...
//use_jcqp          : 0
  ...
  for(int leg = 0; leg < 4; leg++)
  {
    ...
    // Update for WBC 给WBC更新力 //将相关数据统统暴露出 可供外部读取
    Fr_des[leg] = f;
  }
}
